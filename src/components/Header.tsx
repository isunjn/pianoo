...
import { TbPlaylist, TbMusic, TbCrosshair, TbHelp, TbUserCircle, TbMenu2, TbX } from "react-icons/tb";
    ...
    <header className="pt-4 flex justify-between items-end zen-invisible">
      <Link to="/" className="focus-visible:outline-2 focus-visible:outline
        ...
        <div className="text-xl mb-1">
          pianoo
          <span className="ml-3.5 inline-block sm:text-2xl
            sm:group-hover:animate-spin">𝄞</span>
  ...
  return (
    <nav className="relative">
      ...
      {open && <div onClick={() => setOpen(false)}
        className="z-10 fixed top-0 left-0 w-screen h-screen"></div>}
    </nav>
  );
}
